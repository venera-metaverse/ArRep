...
namespace Sample
{
    public class Sample : MonoBehaviour
    {
        ...
        public void FilterBySpotId(InputField text)
        {
            var megaTracker = Session.GetComponentInChildren<MegaTrackerFrameFilter>();
            if (megaTracker.LandmarkFilter.OnNone)
            {
                Debug.LogError("Landmark Filter not exist");
                return;
            }
            megaTracker.LandmarkFilter.Value.FilterBySpotId(text.text, response =>
            {
                if (response.Status == MegaLandmarkFilterStatus.ApiTokenExpired)
                {
        ...
        private void HandleApiTokenExpired()
        {
            ShowMessage(10,
