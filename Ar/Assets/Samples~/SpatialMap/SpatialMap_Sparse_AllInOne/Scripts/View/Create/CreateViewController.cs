...
namespace Sample
{
    public class CreateViewController : MonoBehaviour
    {
        ...
        private BuildMapSession mapSession;
        ...
        private void Update()
        {
            SaveButton.interactable = mapSession.Builder.Target && mapSession.Builder.Target.PointCloud.Count > 20;
        }
        ...
        public void SetMapSession(BuildMapSession session)
        {
            mapSession = session;
        }
        ...
        public void Save()
        {
            ...
            mapSession.Builder.enabled = false;
            Snapshot();
        ...
        public void Snapshot()
        {
            ...
            if (GraphicsSettings.currentRenderPipeline is UnityEngine.Rendering.Universal.UniversalRenderPipelineAsset && !SampleOneShotRendererFeature.IsActive)
            {
                MessageBox.EnqueueMessage("URP RenderPipelineAsset not properly setup! Preview image will be disabled." + Environment.NewLine +
                    Environment.NewLine +
                    $"{typeof(SampleOneShotRendererFeature).Name} not found in current ScriptableRenderer. Please add it to your of RenderPipelineAsset in Editor.", 15);
            }
#endif
            var oneShot = mapSession.ARSession.Assembly.Camera.gameObject.AddComponent<OneShot>();
            oneShot.Shot(OneShotMaterial, true, (texture) =>
            {
                if (capturedImage) { Destroy(capturedImage); }
        ...
        {
            if (!PreviewToggle.interactable) { return; }
